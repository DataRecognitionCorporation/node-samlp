 ...
 * Consumers may supply PKCS#1 ("BEGIN RSA PRIVATE KEY") keys which are
 * accepted by OpenSSL 1.x but trigger "DECODER routines::unsupported" in
 * OpenSSL 3.x when passed through xml-crypto 3.x.  Converting to PKCS#8
 * ("BEGIN PRIVATE KEY") is universally supported.
 * Falls back to the original value if conversion fails.
 */
function normalizePrivateKey(key) {
  if (!key) return key;
  try {
    return nodeCrypto.createPrivateKey(key).export({ type: 'pkcs8', format: 'pem' });
  } catch (e) {
    return key;
  }
}
...
function buildSamlResponse(options) {
  ...
  if (options.signResponse) {
    ...
    sig.signingKey = normalizePrivateKey(options.key);
...
function getSamlResponse(samlConfig, user, callback) {
  ...
  createAssertion.call(saml20, {
    ...
    cert: options.cert,
    key: normalizePrivateKey(options.key),
    issuer: options.issuer,
